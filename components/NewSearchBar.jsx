...
const NewSearchBar = ({varient, page}) => {
    ...
    const updateSearchParams = (params) => {
        ...
        const convertParamsToUrl = (paramso, baseUrl = '/search') => {
  ...
  return (
      <form onSubmit={handleSearch} className={`${varient == "large" ? "s" : "mb-5 md:hidden"}`}>
            <div className="searchBox md:max-h-[50px]">
            <input 
                className={`searchInput font-texts text-white text-lg ${page && page == "main" && "pr-[40vw]"}`}
                type="text" 
                placeholder="Search..."
                value={searchTerm}
                minLength={2} 
                ...
                ref={searchRef}
            />
            <button className="searchButton md:right-0 max-md:right-[8px]" type="submit">
